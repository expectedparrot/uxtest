...
Capture hashes may be abbreviated to seven or more unambiguous characters.
`image-review run` prepares `jobs.ep`, invokes `ep`, and ingests `results.ep`
without requiring the caller to handle paths. For manual control, use
`prepare`, execute its returned `next_command`, and ingest:
...
Screenshots are EDSL `FileStore` values. Ingestion writes structured visual
findings to `analysis/image_review/findings.json`.
...
This writes a self-contained `analysis/journey/journey.svg` plus an HTML viewer,
merging shared run prefixes and branching where recorded actions or outcomes
diverge. Nodes include embedded screenshots, actions, URLs, outcomes, and
supporting run ids. Generated reports embed the SVG when it is available.
